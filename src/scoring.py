...
class ScoringDeck:
    ...
    def increment_player_wins(self, final_counts:dict):
        ...
        WHERE p1 = ? AND p2 = ?
        """
        result = db.run_query(sql, params=(p1, p2))
        current = result.iloc[0]
        print(current)
        print(type(current))

        if p1_tricks > p2_tricks:
            current['p1_wins_tricks'] += 1
        elif p2_tricks > p1_tricks:
            current['p2_wins_tricks'] += 1
        elif p1_tricks == p2_tricks:
            current['draws_tricks'] +=1

        if p1_cards > p2_cards:
            current['p1_wins_cards'] += 1
        elif p2_cards > p1_cards:
            current['p2_wins_cards'] += 1
        elif p1_cards == p2_cards:
            current['draws_cards'] +=1
